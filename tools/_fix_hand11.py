# -*- coding: utf-8 -*-
# 最终修 2007-23 两处：同行 $$\xrightarrow → 拆新块（old 无 \n）
...
for old, new in reps:
    if old in nv:
        ...
        # 尝试无换行版本
        old2 = old.replace('\n', '')
        new2 = '\\end{pmatrix}$$\n$$\nxrightarrow' + old2.split('xrightarrow')[1]
        if old2 in nv:
            nv = nv.replace(old2, new2, 1)
            hit += 1
        else:
            print('W 未命中:', repr(old2[:40]))
if nv != v:
    fail, nseg = rc(nv)
    if fail == 0:
        q['answer'] = nv
        print('OK 2007-23（命中 %d 处）' % hit)
    else:

# -*- coding: utf-8 -*-
"""P1v5：块间裸公式段（无中文，忽略 \\text{...} 内容）→ 包成 $$ 显示块。渲染验证 + 回滚。"""
...
def has_cn(s):
    s2 = re.sub(r'\\text\{[^}]*\}', '', s)
    s2 = re.sub(r'\\operatorname\{[^}]*\}', '', s2)
    return bool(HAN.search(s2))
...
def fix_p1v5(v):
    ...
    for i, p in enumerate(parts):
        if i % 2 == 0:
            s = p
            if (not has_cn(s) and '$' not in s and CMD.search(s)
...
def main():
    ...
    print('P1v5：修好 %d，回滚 %d，跳过 %d（共 %d）' % (ok, len(rb), skip, len(bad_keys)))
